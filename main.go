...
func run() error {
	...
	now := time.Now()
	sunday := int(now.Weekday()) == 0
	if sunday {
		periodTypes = append(periodTypes, "weekly")
	}
	firstDayOfMonth := now.Day() == 1
	if firstDayOfMonth {
		periodTypes = append(periodTypes, "monthly")
	}
	...
	for _, languageType := range languageTypes {
		for _, periodType := range periodTypes {
			...
			//fmt.Printf("https://mkusaka.github.io/trending/%s/%s/index.xml\t%s\t%s\n", languageType, periodType, periodType, languageType)
			fp := gofeed.NewParser()
